...
- **الطلبات** (١٦): موزّعة على كل المراحل (نشط/مغلق/مرفوض) مع سلسلة `workflow_history` كاملة لكل طلب. **الطلب #13 (FINAL) يستخدم نفس رقم الفاتورة INV-2026-10022 كالطلب #3 (INTERNAL) لاختبار تنبيه الفاتورة المكررة.**
- **سجلّات التدقيق** (٢٥): `audit_logs` تجريبية (دخول/إنشاء/تنفيذ/تحديث/تصدير)، idempotent عبر `correlation_id = demo_audit_{i}`.
- **صلاحيات الشاشات**: مصفوفة كاملة لكل الأدوار (٨ أدوار × شاشات) — CR-11 + CR-12.

> **غير مضمّن عمدًا:** بيانات رسوم التقارير (MONTHLY/CATEGORY_DIST) — تُحسب من الطلبات في الخادم، ليست بيانات تُزرع.

## التحديث الأخير (2026-06-25)

- **بيانات الطلبات موسّعة**: `data` JSON الآن يحتوي ٣٠+ حقل (كل حقول النموذج الديناميكي) بدلاً من ٨ حقول فقط
- **أعمدة مخصصة**: `supplier_name`, `import_type`, `country_of_origin`, `goods_description`, `port_of_entry`, `payment_terms`, `invoice_date`, `shipping_port` — يجب أن تكون موجودة في جدول `requests` وفي `$fillable` بالموديل
- **بيانات التاجر**: كل طلب يحتوي `taxNumber`, `linkedCompany`, `commercialRegistration`, `owners` مطابقة لبيانات التاجر المرتبط
- **`requestIdentifier`**: مُضاف في `data` JSON لعرض رقم الطلب في صفحة التفاصيل
- **فاتورة مكررة**: الطلب #13 يستخدم `INV-2026-10022` (مكرر مع #3) لاختبار التنبيه
- **transition_id**: تم إصلاح ربط الـhistory بالـtransitions الصحيحة
- انظر [SEEDER-UPDATE.md](SEEDER-UPDATE.md) للتفاصيل الكاملة
