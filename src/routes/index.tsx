  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={roleCanCreateRequest(user) ? (
          <Button asChild>
            <Link to="/workflows"><FilePlus2 className="h-4 w-4 ml-1" /> طلب جديد</Link>
          </Button>
        ) : undefined}
      />

      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        <Kpi label="ضمن نطاقي" value={stats.total} icon={FileText} tone="text-primary bg-primary/10" />
        <Kpi label="قيد المعالجة" value={stats.active} icon={Clock} tone="text-warning bg-warning/10" />
        <Kpi label="مغلقة" value={stats.closed} icon={CheckCircle2} tone="text-success bg-success/10" />
        <Kpi label="مرفوضة" value={stats.rejected} icon={XCircle} tone="text-destructive bg-destructive/10" />
      </div>
      ...
      <Card className="p-5 shadow-card border-0">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">البيانات معروضة من instances المحرّك، وليس من الطلبات القديمة.</p>
          </div>
          <Link to="/workflows" className="text-xs text-accent hover:underline flex items-center gap-1">
              ...
              <tbody>
                {scoped.slice(0, 8).map((inst) => (
                  <tr key={inst.id} className="border-b last:border-0 hover:bg-muted/40">
                    ...
                    <td className="py-3"><Badge variant="secondary">{stageLabel(inst)}</Badge></td>
                    <td className="py-3 w-36">
                      <Progress value={progressForInstance(inst)} className="h-1.5" />
                      <div className="text-[10px] text-muted-foreground mt-1">{progressForInstance(inst)}%</div>
                    </td>
                    <td className="py-3 text-left">
                      <Button asChild variant="ghost" size="sm">
                        <Link to="/workflows/instances/$id" params={{ id: inst.id }}>فتح</Link>
                      </Button>
                    </td>
                  </tr>
                ))}
